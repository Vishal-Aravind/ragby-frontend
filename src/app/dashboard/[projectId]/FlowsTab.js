...
import { Loader2, Plus, Trash2, ChevronRight, Info } from "lucide-react";
...
const RESERVED_BUTTONS = [
  { id: "ask_a_question", title: "Ask a Question",  desc: "User enters RAG mode. Bot sends confirmation + Back to Menu after every answer." },
  { id: "back_to_menu",   title: "↩ Back to Menu",  desc: "Returns user to the start node of the flow." },
  { id: "talk_to_human",  title: "Talk to Human",   desc: "Triggers human handoff mode." },
];
...
const EMPTY_CONTENT = {
  ...
  rag:     { body: "Ask me anything!", rag_exit_keywords: [] },
...
export default function FlowsTab({ projectId }) {
  ...
  const confirmDeleteFlow = async () => {
    ...
    if (selectedFlow?.id === flowToDelete.id) {
      setSelectedFlow(null); setNodes([]); setEdges([]); setSelectedNode(null);
    }
  ...
  const handleSaveEdge = async (trigger, toNodeId) => {
    ...
    if (!toNodeId) { await fetchNodes(selectedFlow.id); return; }
            ...
            {flows.map(flow => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={e => { e.stopPropagation(); toggleActive(flow); }}>
        ...
        <Card>
          <CardContent className="p-6 space-y-6">
            ...
            <div className="space-y-3 border rounded-lg p-4 bg-gray-50">
              <p className="text-sm font-medium">Flow settings — {selectedFlow.name}</p>
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  placeholder="hi, hello, hey, start, menu"
                />
                <p className="text-xs text-muted-foreground">When user sends any of these → flow starts</p>
              </div>
              ...
              <div className="flex items-start gap-3">
                <input
                  type="checkbox"
                  id="free_questions"
                  checked={editFreeQuestions}
                  onChange={e => setEditFreeQuestions(e.target.checked)}
                  className="mt-0.5"
                />
                <div>
                  <label htmlFor="free_questions" className="text-sm font-medium cursor-pointer">
                    Allow free questions
                  </label>
                  <p className="text-xs text-muted-foreground mt-0.5">
                    When ON — if user types a message instead of clicking a button, AI answers and then resends the buttons.
                    When OFF — typed messages are ignored and buttons are resent silently.
                  </p>
                  {editFreeQuestions && (
                    <p className="text-xs text-blue-600 mt-1">
                      ✓ Welcome message will auto-include: "Tap an option or type your question directly"
                    </p>
                  )}
                </div>
              </div>
              ...
              <Button size="sm" onClick={handleSaveSettings} disabled={savingSettings}>
                {savingSettings ? <Loader2 size={13} className="animate-spin mr-1" /> : null}
                Save settings
              </Button>
            </div>

            {/* ── Reserved button IDs hint ── */}
            <div className="border rounded-lg overflow-hidden">
              <button
                className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium bg-amber-50 hover:bg-amber-100 transition-colors"
                onClick={() => setShowReserved(r => !r)}
              >
                <div className="flex items-center gap-2">
                  <Info size={14} className="text-amber-600" />
                  <span className="text-amber-800">Special button IDs — use these in your buttons/list nodes</span>
                </div>
                <span className="text-amber-600 text-xs">{showReserved ? "Hide" : "Show"}</span>
              </button>
              {showReserved && (
                <div className="divide-y">
                  {RESERVED_BUTTONS.map(rb => (
                    <div key={rb.id} className="px-4 py-3 flex items-start gap-3">
                      <code className="text-xs bg-gray-100 px-2 py-1 rounded font-mono shrink-0">{rb.id}</code>
                      <div>
            ...
            <div className="space-y-3">
              ...
              {nodes.length === 0 && (
                <p className="text-sm text-muted-foreground">No nodes yet. Click "Add node" to start building.</p>
              )}
                  ...
                  <div className="border rounded-lg p-4 space-y-4">
                    ...
                    {editType === "buttons" && (
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          <p className="text-xs text-muted-foreground">Buttons (max 3) — use special IDs from the hint above for AI/handoff/back</p>
                          {(editContent.buttons || []).map((btn, idx) => (
                            <div key={idx} className="space-y-1 border rounded p-2">
                              <div className="flex gap-2 items-center">
                                ...
                                />
                                <Input
                                  placeholder="ID (e.g. ask_a_question)"
                                  value={btn.id}
                              ...
                              {/* Only show "goes to" if not a reserved ID */}
                              {!RESERVED_BUTTONS.find(r => r.id === btn.id) && btn.id && (
                                <div className="flex items-center gap-2">
                              ...
                              {RESERVED_BUTTONS.find(r => r.id === btn.id) && (
                                <p className="text-xs text-amber-600">
                                  ✓ Reserved — {RESERVED_BUTTONS.find(r => r.id === btn.id)?.desc}
                                </p>
                              )}
                            </div>
                          ))}
                            ...
                            {section.rows.map((row, rIdx) => (
                              <div key={rIdx} className="flex gap-2 items-start">
                                <div className="flex-1 space-y-1">
                                  ...
                                  {RESERVED_BUTTONS.find(r => r.id === row.id) && (
                                    <p className="text-xs text-amber-600">
                                      ✓ Reserved — {RESERVED_BUTTONS.find(r => r.id === row.id)?.desc}
                                    </p>
                                  )}
                                </div>
                    ...
                    {editType === "rag" && (
                      <div className="space-y-3">
                        ...
                        <p className="text-xs text-muted-foreground bg-amber-50 border border-amber-200 rounded p-2">
                          ℹ️ In RAG mode, the user can only ask questions. Every answer gets a [Back to Menu] button automatically. No special keywords work here.
                        </p>
                      </div>
                    )}
                    ...
                    {editType === "handoff" && (
                      <div className="space-y-1">
                        ...
                        />
                        <p className="text-xs text-muted-foreground bg-amber-50 border border-amber-200 rounded p-2 mt-2">
                          ℹ️ You can also trigger handoff via a button with ID <code className="font-mono">talk_to_human</code> — no need for a separate handoff node.
                        </p>
                      </div>
                    )}
